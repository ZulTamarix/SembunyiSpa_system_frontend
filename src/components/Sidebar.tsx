...
const sections: NavSection[] = [
  {
    title: "OVERVIEW",
    items: [
      { label: "Dashboard", path: "/dashboard", icon: <LayoutGrid size={18} />, icon_selected: <LayoutGrid size={18} className="text-emerald-400" /> },
    ],
  },
  {
    title: "OPERATIONS",
...
const Sidebar: React.FC<SidebarProps> = ({ isOpen = true }) => {
  ...
  // for logout only
  const logoutItem: NavItem = { label: "Sign Out", path: "/logout", icon: <CornerDownLeft size={18} />, icon_selected: <CornerDownLeft size={18} /> };
                    ...
                    <button
                      ...
                      className={`w-full flex items-center gap-3 py-2.5 text-sm transition-colors relative ${
                        ...
                        active
                          ...
                          : "text-white/60 hover:text-white/90 hover:bg-highlighted"
                      }`}
                    >
                      {active ? (
                        <>
                          <span className="absolute left-0 top-0 h-full w-0.75 bg-amber-400 rounded-r" />
                          <span className="shrink-0 text-white/70">{item.icon_selected}</span>
                        </>
                      ) : (
